...
function NewSessionScreen() {
    ...
    // Recents lead the list, so the auto-select effect below picks the last
    // used path instead of whatever sorts first (which was always the home
    // directory). Non-recent places keep collectSessionPlaces' order.
    const pathItems = React.useMemo<PickerItem[]>(() => {
        const recents = selectedMachineId ? recentPathsByMachine[selectedMachineId] ?? [] : [];
        const recentRank = new Map(recents.map((p, i) => [normalizePathForComparison(p, selectedHomeDir), i] as const));
        const ordered = [...places].sort((a, b) => {
            const ra = recentRank.get(normalizePathForComparison(a.path, selectedHomeDir)) ?? Number.MAX_SAFE_INTEGER;
            const rb = recentRank.get(normalizePathForComparison(b.path, selectedHomeDir)) ?? Number.MAX_SAFE_INTEGER;
            return ra - rb;
        });
    ...
    }, [places, recentPathsByMachine, selectedHomeDir, selectedMachineId]);
    ...
    }, [allMachines, canPickWorktree, currentEffort?.key, currentModelKey, currentPermission?.key, effectiveAgentDefaults.effortLevel, effectiveAgentDefaults.modelMode, effectiveAgentDefaults.permissionMode, experiments, navigateToSession, router, selectedAgent, selectedMachineId, selectedMachine, selectedPath, worktreeKey, rigCreation, supportsWorktree, expImageUpload, clearImages, recentPathsByMachine, setRecentPathsByMachine]);
