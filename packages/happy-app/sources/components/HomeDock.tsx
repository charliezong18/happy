...
export const HomeDock = React.memo(({
    ...
    const selectAgent = React.useCallback((agent: NewSessionAgentType) => {
        ...
        // defaults — the defaults memo above covers rig via rigCreation)
        // instead of overwriting them. Upstream seeds defaults here instead;
        // that clobbers the restored per-agent modes. The rig machine switch
        // that used to live in an effect here is gone with upstream's computer
        // grouping: one computer covers both daemons, and creation routes to
        // the right half.
        setAgentType(agent);
    }, [setAgentType]);
